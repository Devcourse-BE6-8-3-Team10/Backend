...
class TradeService (
    ...
    fun createTrade(postId: Long, buyerId: Long): Trade {
        ...
        if (post.status == Post.Status.SOLD_OUT)
            throw ServiceException("400-1", "이미 판매된 게시글입니다.")

        if (post.member.id == buyerId)
            throw ServiceException("403-1", "자신의 게시글은 구매할 수 없습니다.")
    ...
    fun getMyTrades(member: Member, pageable: Pageable): Page<TradeDto> =
        tradeRepository.findByBuyerOrSeller(member, member, pageable)
            .map { TradeDto(it) }
    ...
    fun getTradeDetail(id: Long, member: Member): TradeDetailDto {
        val trade = tradeRepository.findById(id)
            .orElseThrow { ServiceException("404-1", "거래를 찾을 수 없습니다.") }

        if (trade.buyer.id != member.id && trade.seller.id != member.id) {
            throw ServiceException("403-1", "본인의 거래만 조회할 수 있습니다.")
        }
